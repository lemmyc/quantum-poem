...
const SunModel = ({
  ...
  const generateNewKeywords = useCallback(async () => {
    try {
      ...
      if (newSortedWords.length === 0) {
        throw new Error(
          "Could not determine relevant keywords from the generated list."
        );
      }
      ...
      electronsRef.current.forEach((electron, index) => {
        if (electron.mesh.children[0]) {
          ...
          ctx.fillStyle = "#fff";
          ctx.fillText(
            newSortedWords[index]?.word || `Electron ${index + 1}`,
            128,
            32
          );
  ...
  const fetchAndProcessProbabilities = useCallback(async () => {
    const currentElectronLabels =
      Array.isArray(electronLabelsRef.current) &&
      electronLabelsRef.current.length >= 3
      ...
      if (data && data.results && Array.isArray(data.results)) {
        ...
        if (sortedResults.length > 0) {
          if (
            ...
            sortedResults[0].word === lastHighlightedWordRef.current
          ) {
            wordToHighlight =
              sortedResults.length > 1 ? sortedResults[1].word : null;
          } else {
        ...
        if (wordToHighlight) {
          ...
          const indexToHighlight = electronTexts.findIndex(
            (label) => label === wordToHighlight
          );

          if (indexToHighlight !== -1 && currentElectrons[indexToHighlight]) {
            electronToHighlightRef.current = currentElectrons[indexToHighlight];
            setConfirmModalText(
              `Text phù hợp với cảm xúc của bạn là ${wordToHighlight}`
            );
            setTimeout(() => {
  ...
  const handleConfirmOk = () => {
    if (electronToHighlightRef.current) {
      ...
      electronToHighlightRef.current.mesh.material.emissive.copy(
        highlightColor
      );
  ...
  useEffect(() => {
    ...
    loader.load(
      ...
      (gltf) => {
        ...
        const NUM_ORBITS = 3;
        const currentElectronDisplayLabels =
          Array.isArray(electronLabels) && electronLabels.length > 0
        ...
        for (let i = 0; i < NUM_ORBITS; i++) {
          ...
          glowOrbit.rotation.set(x, y, z);
          glowOrbit.renderOrder = 2;
          orbitGroup.add(glowOrbit);

          const glowGeometry2 = new THREE.TubeGeometry(
            path,
            3200,
            0.32,
            16,
          ...
          glowOrbit2.rotation.set(x, y, z);
          glowOrbit2.renderOrder = 3;
          orbitGroup.add(glowOrbit2);

          const glowGeometry3 = new THREE.TubeGeometry(
            path,
            4000,
            0.65,
            64,
          ...
          for (let j = 0; j < numElectronsOnOrbit; j++) {
            ...
            ctx.fillText(
              currentElectronDisplayLabels[electronIndex] ||
                `Electron ${electronIndex + 1}`,
              128,
              32
            );
            const textTexture = new THREE.CanvasTexture(canvas);
  ...
  useEffect(() => {
    if (
      isModelLoaded &&
      electronLabels.length > 0 &&
      !initialProbFetchDone.current
    ) {
      fetchAndProcessProbabilities();
  ...
  return (
    <div className="sun-model-container">
      <div
        className={`sun-model ${isMovingToCorner ? "move-to-corner" : ""}`}
        ref={mountRef}
      >
        <Modal
